--------------------------------------------------------
--  DDL for Procedure cargarDatosALM_MES
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ALM_REPORT"."cargarDatosALM_MES" AS
BEGIN
execute immediate 'TRUNCATE TABLE ALM_REPORT.DATOS_ALM_MES';
...
execute immediate  'insert into ALM_REPORT.DATOS_ALM_MES 
SELECT
...
TC.TC_EXEC_DATE >= TO_DATE(''01/''||''02''||''/2016'',''dd/mm/yyyy'')   AND
...
RN.RN_EXECUTION_DATE >= TO_DATE(''01/''||''02''||''/2016'',''dd/mm/yyyy'') AND
...
CASE WHEN  TO_DATE(TO_CHAR(sysdate, ''yyyy-mm-dd''), ''yyyy-mm-dd'')  <= TO_DATE(''08/''||TO_CHAR(SYSDATE,''mm'')||''/2016'', ''dd/mm/yyyy'')
THEN
TO_DATE(''01/''||(TO_CHAR(SYSDATE,''mm'') - 1)||''/2016'', ''dd/mm/yyyy'')
ELSE
TO_DATE(''01/''||(TO_CHAR(SYSDATE,''mm''))||''/2016'', ''dd/mm/yyyy'')
END 
...
for x in (select DB_NAME, PROJECT_NAME, DOMAIN_NAME from alm_siteadmin_db_12.projects where domain_name in ('bch_qa_certificación_bch_qa_d') ) loop
execute immediate  'insert into ALM_REPORT.DATOS_ALM_MES 
SELECT
'''||x.db_name||'.'' || R.REL_ID "Id Version",
R.REL_NAME "Nombre Version",
...
R.REL_USER_TEMPLATE_01 as jdp_qa,
...
LEFT JOIN  '||x.db_name||'.TESTCYCL TC ON TC.TC_ASSIGN_RCYC = RC.RCYC_ID
LEFT JOIN  '||x.db_name||'.RUN RN ON RN.RN_TESTCYCL_ID = TC.TC_TESTCYCL_ID 
INNER JOIN  '||x.db_name||'.TEST TS ON TS.TS_TEST_ID=TC.TC_TEST_ID
INNER JOIN '||x.db_name||'.ALL_LISTS AL ON TS.TS_SUBJECT = AL.AL_ITEM_ID

WHERE

RC.RCYC_NAME not LIKE (''%IDC%'') and   AL.AL_ABSOLUTE_PATH LIKE ''%AAAAAPAADAAR%'' AND 
