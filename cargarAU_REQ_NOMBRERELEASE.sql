--------------------------------------------------------
--  DDL for Procedure cargarAU_REQ_NOMBRERELEASE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ALM_REPORT"."cargarAU_REQ_NOMBRERELEASE" AS
BEGIN
execute immediate 'TRUNCATE TABLE ALM_REPORT.AU_REQ_NOMBRERELEASE';
...
execute immediate  'insert into ALM_REPORT.AU_REQ_NOMBRERELEASE
SELECT
DISTINCT 
RQ.RQ_REQ_ID "ID REQ",
...
LEFT JOIN '||x.DB_NAME||'.REQ_RELEASES RQRL ON RQRL_REQ_ID = RQ.RQ_REQ_ID
LEFT JOIN '||x.DB_NAME||'.RELEASES R ON R.REL_ID = RQRL_RELEASE_ID

WHERE 
RQ.RQ_TYPE_ID = 101
AND
RQ.RQ_REQ_NAME != R.REL_NAME
