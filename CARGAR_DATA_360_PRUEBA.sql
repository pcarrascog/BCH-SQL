--------------------------------------------------------
--  DDL for Procedure CARGAR_DATA_360_PRUEBA
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ALM_REPORT"."CARGAR_DATA_360_PRUEBA" AS 
BEGIN
execute immediate 'TRUNCATE TABLE ALM_REPORT.DATA_360_PRUEBA';
...
execute immediate  'insert into ALM_REPORT.DATA_360_PRUEBA 
select 
...
-----------------------------------------      
      ...
      LEFT JOIN '||x.db_name||'.RELEASE_FOLDERS RF ON  RF.RF_ID = RPROM.REL_PARENT_ID 
        ...
        prom_añejamiento1        
             ...
             group by RPROM.REL_ID ) defec3 on defec3.rel_id = rel.rel_id
...
--Trae la cantidad de defectos Software
            ...
            JOIN '||x.db_name||'.RELEASE_CYCLES ON BG_TARGET_RCYC = RCYC_ID
            WHERE 1=1
            AND b.BG_USER_06 IN (''Error de Codificación - Funcional'', ''Error de Diseño'')
            AND RCYC_NAME NOT LIKE ''%IDC%'' 
            group by rel_id, RCYC_ID) defec4 on defec4.rel_id = rel.rel_id and RCYC.RCYC_ID = defec4.RCYC_ID            
-----------------------------------------
...
--Trae la cantidad de defectos Diseño
            ...
            JOIN '||x.db_name||'.RELEASE_CYCLES ON BG_TARGET_RCYC = RCYC_ID
            WHERE 1=1
            AND b.BG_USER_06 IN (''Error de Análisis de Amiguedad'')
            AND RCYC_NAME NOT LIKE ''%IDC%'' 
            group by rel_id, RCYC_ID) defec4 on defec4.rel_id = rel.rel_id and RCYC.RCYC_ID = defec4.RCYC_ID            
-----------------------------------------
...
REL.REL_START_DATE          "Fecha Inicio Re-Plan Release",
REL.REL_END_DATE            "Fecha Fin Re-Plan Release",
DIAS_LABORABLES( TO_DATE(REL.REL_USER_01, ''YYYY/MM/DD''),TO_DATE(REL.REL_USER_02, ''YYYY/MM/DD'')) "Dias rel plan", 
DIAS_LABORABLES( REL.REL_START_DATE,TO_DATE(REL.REL_USER_01, ''YYYY/MM/DD'')) "Defase Inicio Rel", 
RCYC.RCYC_ID                "Id Ciclo", 
RCYC.RCYC_NAME              "Nombre Ciclo",
TO_DATE(RCYC.RCYC_USER_01, ''YYYY/MM/DD'') "Fecha Inicio Plan Ciclo",
...
nvl (total_def_Urgent, 0),
nvl (total_def_Ambiente, 0) AS TOT_AMBIENTE,
nvl (total_def_Eje_Auto, 0) AS TOT_EJE_AUTO,
nvl (total_def_Eliminar, 0) AS TOT_ELIMINAR,
nvl (total_def_Diseño, 0) AS TOT_DISEÑO,
nvl (total_def_Software, 0) AS TOT_SOFTWARE,
''SERVIDOR BCH'' as SERVIDOR
...
-----------------------------------------
      LEFT  JOIN(
      SELECT re.rel_id, COUNT (b.BG_BUG_ID) as Cant_def, Cant_iteraciones, (Cant_iteraciones/COUNT (b.BG_BUG_ID)) prom_iteraciones
      FROM  '||x.db_name||'.BUG b
      JOIN '||x.db_name||'.RELEASES re ON b.BG_TARGET_REL = re.REL_ID      
      JOIN '||x.db_name||'.RELEASE_CYCLES ON BG_TARGET_RCYC = RCYC_ID
...
--Trae la cantidad de defectos Ambiente
      LEFT  JOIN(
            ...
            JOIN '||x.db_name||'.RELEASE_CYCLES ON BG_TARGET_RCYC = RCYC_ID
            WHERE 1=1
            AND b.BG_USER_06 IN (''Error Ambigüedad requerimiento'', ''Error ambiente hardware banco'', ''Error de Ambiente Software'')
            AND RCYC_NAME NOT LIKE ''%IDC%'' 
      group by rel_id, RCYC_ID) defec4 on defec4.rel_id = rel.rel_id and RCYC.RCYC_ID = defec4.RCYC_ID
-----------------------------------------


--Trae la cantidad de defectos Ejecuciones Automaticas
      LEFT  JOIN(
            SELECT 
            rel_id, RCYC_ID, COUNT(BG_BUG_ID) as total_def_Eje_Auto
            FROM '||x.db_name||'.BUG b
            JOIN '||x.db_name||'.RELEASES ON b.BG_TARGET_REL = REL_ID      
            JOIN '||x.db_name||'.RELEASE_CYCLES ON BG_TARGET_RCYC = RCYC_ID
            WHERE 1=1
            AND b.BG_USER_06 IN (''Error construcción script/error automatización'')
            AND RCYC_NAME NOT LIKE ''%IDC%'' 
      group by rel_id, RCYC_ID) defec4 on defec4.rel_id = rel.rel_id and RCYC.RCYC_ID = defec4.RCYC_ID
-----------------------------------------


--Trae la cantidad de defectos Ejecuciones Software
      LEFT  JOIN(
            SELECT 
            rel_id, RCYC_ID, COUNT(BG_BUG_ID) as total_def_Software
            FROM '||x.db_name||'.BUG b
...
--Trae la cantidad de defectos Ejecuciones Diseño
      LEFT  JOIN(
            SELECT 
            rel_id, RCYC_ID, COUNT(BG_BUG_ID) as total_def_Diseño
            FROM '||x.db_name||'.BUG b
            ...
            JOIN '||x.db_name||'.RELEASE_CYCLES ON BG_TARGET_RCYC = RCYC_ID
            WHERE 1=1
            AND b.BG_USER_06 IN (''Error de Análisis de Ambiguedad'')
            AND RCYC_NAME NOT LIKE ''%IDC%'' 
      group by rel_id, RCYC_ID) defec4 on defec4.rel_id = rel.rel_id and RCYC.RCYC_ID = defec4.RCYC_ID
-----------------------------------------

--Trae la cantidad de defectos Ejecuciones Eliminar
      LEFT  JOIN(
            SELECT 
            rel_id, RCYC_ID, COUNT(BG_BUG_ID) as total_def_Eliminar
            FROM '||x.db_name||'.BUG b
            ...
            JOIN '||x.db_name||'.RELEASE_CYCLES ON BG_TARGET_RCYC = RCYC_ID
            WHERE 1=1
            AND b.BG_USER_06 IN (''Error Copia caso automático'', ''Error ambiente Hardware laboratorio'', ''Error diseño Caso de prueba (pauta manual)'', ''Error diseño requerimiento/proyecto'', ''Error de Parámetros'', ''Error de Procesamiento de Interfaces'', ''Error de Pruebas'', ''Error de Requerimiento'', ''Error por Omisión'', ''QTP'')
            AND RCYC_NAME NOT LIKE ''%IDC%'' 
      group by rel_id, RCYC_ID) defec4 on defec4.rel_id = rel.rel_id and RCYC.RCYC_ID = defec4.RCYC_ID
-----------------------------------------
...
prom_añejamiento1        
                  ...
                  LEFT JOIN (SELECT BG_BUG_ID as id_BG ,
                                  rel_id , 
                                  --(nvl( sum (DIAS_LABORABLES (bg_detection_date,AU_TIME))/ count (distinct bg_bug_id)-1,0)) as prom_añejamiento2 
                                  MAX( (DIAS_LABORABLES (bg_detection_date,AU_TIME))) as prom_añejamiento2               
                                  FROM '||x.db_name||'.BUG
                                  ...
                                  JOIN '||x.db_name||'.RELEASES  ON BG_TARGET_REL = REL_ID 
                                  WHERE AU_ENTITY_TYPE = ''BUG''                
                                  AND AP_NEW_VALUE  IN (''Closed'', ''Canceled'')
                      AND AP_PROPERTY_NAME like ''%Estado%''		
                                  --and rel_id = 1002
                                  AND RCYC_NAME NOT LIKE ''%IDC%''
                                  group by rel_id, BG_BUG_ID ) BH ON BH.id_BG =  B.BG_BUG_ID               
                  where 1=1
                  --and RA.rel_id = 1002
             ...
             group by RPROM.REL_ID  
                
                ) defec3 on defec3.rel_id = rel.rel_id
...
TO_DATE (REL1.REL_USER_01,''YYYY/MM/DD'') <= MIN(REL1.REL_START_DATE) THEN
(DIAS_LABORABLES(TO_DATE (REL1.REL_USER_01,''YYYY/MM/DD''), MIN(REL1.REL_START_DATE)))-1 ELSE -(DIAS_LABORABLES(MIN(REL1.REL_START_DATE), TO_DATE (REL1.REL_USER_01,''YYYY/MM/DD'')))  END) AS desfase_rel,               
--              nvl( (DIAS_LABORABLES (TO_DATE(REL1.REL_USER_01, ''YYYY/MM/DD''),MIN(REL1.REL_START_DATE)))-1,0) desfase_rel,
                SUM((desfase_run_cicl))as desfase_inicio
                FROM '||x.db_name||'.RELEASES REL1                             
                JOIN '||x.db_name||'.RELEASE_CYCLES RC ON REL1.REL_ID = RC.RCYC_PARENT_ID
                                ...
                                --AND REL_ID = ''1002''
                                AND RCYC_NAME NOT LIKE ''%IDC%''
                                GROUP BY REL_ID, RCYC_ID, REL_USER_01, REL_START_DATE, RCYC_USER_01) Des_ini ON REL1.REL_ID = Des_ini.REL_ID and Des_ini.RCYC_ID = RC.RCYC_ID                     
                                WHERE 1=1
                                --AND RCYC_NAME LIKE ''%uncional%''
				--AND REL1.REL_ID = ''1002''
				AND RCYC_NAME NOT LIKE ''%IDC%''
				GROUP BY REL1.REL_ID, REL1.REL_USER_01, REL1.REL_START_DATE) des_ini1 on rel.rel_id = des_ini1.rel_id
