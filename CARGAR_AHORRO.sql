--------------------------------------------------------
--  DDL for Procedure CARGAR_AHORRO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ALM_REPORT"."CARGAR_AHORRO" AS 
BEGIN
 execute immediate 'TRUNCATE TABLE ALM_REPORT.BCH_CONSOLIDADO_AHORRO';
...
execute immediate  'insert into ALM_REPORT.BCH_CONSOLIDADO_AHORRO 
...
INNER JOIN '||x.db_name||'.REQ RQ ON RQ.RQ_REQ_ID=RQR.RQRL_REQ_ID
      ...
      WHERE REL_ID IN (''1812'')
      )tabla2 on tabla2.id_ciclo = RCYC.RCYC_ID

    
WHERE 1=1
AND REL.REL_ID IN (''1812'')
...
execute immediate  'insert into ALM_REPORT.BCH_CONSOLIDADO_AHORRO 
select * from BCH_CONSOLIDADO_AHORRO@SERVER_BCH
';
